        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('appointments.update', $appointment) }}" method="POST" id="appointment-form" class="space-y-4">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <select name="doctor_id" id="doctor_id" class="border rounded p-2 w-full" required>
                                ...
                                @foreach($doctors as $doc)
                                    <option value="{{ $doc->id }}" {{ (int) old('doctor_id', $appointment->doctor_id) === (int) $doc->id ? 'selected' : '' }}>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input
                                    ...
                                    value="{{ old('appointment_date', optional($appointment->appointment_date)->toDateString() ?? (is_string($appointment->appointment_date) ? $appointment->appointment_date : '')) }}"
                            ...
                            <div>
                                ...
                                <small class="text-gray-500">Only available and unlocked slots are listed. Current slot remains selectable.</small>
                            </div>
                        </div>

                        <div>
                            <label class="block text-sm font-medium mb-1">Reason</label>
                            <textarea name="reason" rows="3" class="border rounded p-2 w-full" required>{{ old('reason', $appointment->reason) }}</textarea>
                        </div>

                        <div>
                            <label class="block text-sm font-medium mb-1">Notes (optional)</label>
                            <textarea name="notes" rows="2" class="border rounded p-2 w-full">{{ old('notes', $appointment->notes) }}</textarea>
                        </div>
                    ...
                    <script>
                        ...
                        async function refreshTimes() {
                            ...
                            times.forEach(t => {
                                const disabled = (booked.has(t) || locked.has(t)) && t !== currentTime;
                                if (!disabled) options.push(`<option value="${t}" ${t === currentTime ? 'selected' : ''}>${t}</option>`);
                            });

                            // Ensure current time is present even if outside standard window
                            if (currentTime && !options.some(o => o.includes(`value="${currentTime}"`))) {
                                options.push(`<option value="${currentTime}" selected>${currentTime}</option>`);
                            }
                        ...
                        // Initialize date value if empty
                        if (!dateEl.value) {
                            try {
                                dateEl.value = `{{ old('appointment_date', optional($appointment->appointment_date)->toDateString() ?? (is_string($appointment->appointment_date) ? $appointment->appointment_date : '')) }}`;
                            } catch (_) {}
                        }
